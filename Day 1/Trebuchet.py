...
letters = dict(reversed(letters.items()))
print(letters)
...
def replace(input):
    output = input
    for digit in letters.keys():
        output = output.replace(digit, str(letters.get(digit)))
    return output

def text(input) :
    lines = input.split(None)

    total = 0
    for line in lines:
        digit = replace(line)
        print(digit)
        total += int(digits(digit))
